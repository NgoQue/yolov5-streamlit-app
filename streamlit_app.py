...
uploaded_file = st.sidebar.file_uploader('Please upload your TEM image to the box below.', type=['png', 'jpeg', 'jpg'])
...
st.image("Data_dielectric_function/coreshell.jpg", use_container_width=True)
    ...
    if __name__ == '__main__':
        ...
        if not np.isnan(D_shell):
            with col1:
                ...
                plt.figure(dpi=300)
                plt.hist(diameter_shell1, bins=10, color='b', alpha=0.7)
                plt.ylabel('Frequency', fontsize=14)
                plt.xlabel('Size', fontsize=14)
